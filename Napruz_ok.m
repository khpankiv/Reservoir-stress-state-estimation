...
for k=1:nn
    ss1g(k,1:nm)=splinezgl(j,jj,ss1g1(k,:),p0,0,2)';
...
rr1n=rr1g;
for i=1:nm
    qwe=(sign(rr2g22(:,i))-sign(rr1g22(:,i)));
    r1=rr1g(:,i)';
    r2=rr2g(:,i)';
    sg=ss1g(:,i)';
    r1n=r1;
    ...
    for k=1:nn
        if (qwe(k)~=0 )
            ...
            pq(k)=kut*(sg(k)-sg(i1))+r1(i1);
            kut2=tan(2*atan(kut)-pi/2);
            ry((i1):(in))=kut2*(sg((i1):(in))-sg(k))+pq(k);
            [rizn,ii]=min(abs(ry((i1):(in))-r1((i1):(in))));
            ii=ii+i1-1;
            d=sqrt((r1(ii)-pq(k))^2+(sg(ii)-sg(k))^2);
            r1n(k)=pq(k)+sign(pq(k)-r1(k)).*d;
        end
    end
    rr1n(:,i)=r1n;
    figure(i)
    plot(sg,r1,'b',sg,r2,'r',sg,r1n,'b*',sg,r2-r1,'g')
    disp(i)
    disp(max(abs(r2-r1n)))
end
%end hlopun
